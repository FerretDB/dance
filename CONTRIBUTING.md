...
Databases under test (FerretDB and MongoDB) may be running on the host or inside Docker; Docker Compose configuration is provided for convenience but not required. <!-- TODO: Is that still relevant? mongodb container now is needed to run `mongodump`-->
